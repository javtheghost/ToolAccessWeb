...
@Component({
  ...
  template: `
    <div class="rounded-xl bg-white shadow p-4 flex flex-col gap-2 h-full">
      ...
      <div class="bg-blue-50 rounded-lg p-2 flex flex-row items-center justify-between">
        ...
        <div class="flex flex-col items-end ml-4 min-w-[80px]">
          ...
          <div class="text-xs font-semibold" [ngClass]="trend === 'up' ? 'text-blue-500' : 'text-red-500'">
            {{ trend === 'up' ? '↗' : '↘' }} {{ percentage }}%
          </div>
        </div>
      </div>
...
export class StatsCardBlueComponent {
  @Input() title: string = 'Herramientas Activas';
  ...
  private getColor(): string {
    ...
    return colors[this.color as keyof typeof colors] || '#2563eb';
  }
}
