...
@Component({
  ...
  template: `
    <div class="rounded-xl bg-white shadow p-4 flex flex-col gap-2 h-full">
      ...
      <div class="bg-red-50 rounded-lg p-2 flex flex-row items-center justify-between">
        ...
        <div class="flex flex-col items-end ml-4 min-w-[80px]">
          ...
          <div class="text-xs font-semibold" [ngClass]="trend === 'up' ? 'text-red-500' : 'text-green-500'">
            {{ trend === 'up' ? '↗' : '↘' }} {{ percentage }}%
          </div>
        </div>
      </div>
...
export class StatsCardRedComponent {
  @Input() title: string = 'Multas Pendientes';
  @Input() value: number = 0;
  @Input() percentage: number = 0;
  @Input() trend: 'up' | 'down' = 'down';
  ...
  private getColor(): string {
    ...
    return colors[this.color as keyof typeof colors] || '#ef4444';
  }
}
